...
export default function App() {
    ...
    try {
      chrome.storage.local.get(["enabled"], (result) => {
        if (chrome.runtime?.lastError) {
          console.warn(
            "[Verbo CRX Content] Storage access error (Extension context invalidated):",
        ...
        console.log(
          `[Verbo CRX Content] Loaded initial config. Status: ${enabled ? "ENABLED" : "DISABLED"}`
        )
      ...
      const handleStorageChange = (
        ...
        if (areaName === "local" && changes.enabled !== undefined) {
          ...
          console.log(
            `[Verbo CRX Content] Config changed. Status updated to: ${newEnabled ? "ENABLED" : "DISABLED"}`
          )
      ...
      console.warn(
        "[Verbo CRX Content] Extension context invalidated on load. Please refresh this tab.",
        err
      )
    }
  }, [])
  ...
  useEffect(() => {
    ...
    const handleInput = (e: Event) => {
      ...
      if (rootEl && !document.body.contains(rootEl)) {
        console.log(
          "[Verbo CRX Content] Re-attaching root container to document.body..."
        )
      ...
      if (suggestionRef.current) {
        console.log(
          "[Verbo CRX Content] User resumed typing. Hiding existing suggestion."
        )
        setSuggestion("")
      }
      ...
      debounceTimerRef.current = setTimeout(() => {
        console.log(
          `[Verbo CRX Content] Typing paused for ${DEBOUNCE_MS}ms. Initiating AI completion request...`
        )
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Tab") {
        ...
        e.stopPropagation()
        console.log(
          `[Verbo CRX Content] Keypress: Tab. Accepting AI suggestion: "${currentSuggestion}"`
        )
        acceptSuggestion()
        ...
        e.stopPropagation()
        console.log(
          "[Verbo CRX Content] Keypress: Escape. Dismissing AI suggestion."
        )
        setSuggestion("")
      }
    }
    ...
    const handleFocusOut = (e: FocusEvent) => {
      ...
      setTimeout(() => {
        if (activeElRef.current === e.target) {
          if (suggestionRef.current) {
            console.log(
              "[Verbo CRX Content] Active element lost focus. Hiding suggestion."
            )
          }
          setSuggestion("")
  ...
  const requestSuggestion = (text: string, targetEl: HTMLElement) => {
    console.log(
      `[Verbo CRX Content] Dispatching message to background worker -> { action: 'getAISuggestion', text: "${text}" }`
    )
    try {
      chrome.runtime.sendMessage(
        { action: "getAISuggestion", text },
        (response) => {
          if (chrome.runtime?.lastError) {
            console.warn(
              "[Verbo CRX Content] Connection error (Extension context invalidated. Please refresh this tab):",
              chrome.runtime.lastError.message
            )
            return
          }
          ...
          if (!currentTarget || !document.contains(currentTarget)) {
            console.log(
              "[Verbo CRX Content] Original target no longer in DOM. Inspecting document.activeElement..."
            )
            if (
              document.activeElement &&
              isTargetable(document.activeElement as HTMLElement)
            ) {
              currentTarget = document.activeElement as HTMLElement
            } else {
              console.log(
                "[Verbo CRX Content] Active element is not targetable. Discarding suggestion."
              )
              return
            }
          }
          ...
          if (response && response.error) {
            console.error(
              "[Verbo CRX Content] Error returned from background AI worker:",
              response.error
            )
            setSuggestion("")
            return
          ...
          if (
            ...
            console.log(
              `[Verbo CRX Content] Success! Rendering suggestion pill on UI: "${response.suggestion}"`
            )
            ...
            console.log(
              "[Verbo CRX Content] AI returned empty continuation. Nothing to render."
            )
            setSuggestion("")
          }
        }
      ...
      console.warn(
        "[Verbo CRX Content] Failed to send message. Extension context invalidated. Please refresh this tab.",
        err
      )
    }
  }
  ...
  const isTargetable = (el: HTMLElement) => {
    ...
    return (
      (isInput || isTextarea || isContentEditable) &&
      !(el as any).readOnly &&
      !(el as any).disabled
    )
  }

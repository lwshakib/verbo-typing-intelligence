...
export async function getAISuggestions(
  ...
  try {
    if (!config.model.startsWith('gemini-')) {
      console.warn('[AI] Only Google Gemini models are supported currently. Model:', config.model);
      return { suggestion: '' };
    }
...
5. If you cannot provide a logical continuation, return an empty string.`;
    ...
    console.log(`[AI] Requesting completion with model: ${config.model} using official Google Gen AI SDK`);
